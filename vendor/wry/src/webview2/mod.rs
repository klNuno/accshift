...
impl InnerWebView {
  ...
  fn new_in_hwnd(
    ...
    // Accshift patch: take over the WebView2 start made by prestart::start.
    let prestarted = if !is_child && pl_attrs.environment.is_none() && prestart::is_pending() {
      let key = prestart::env_key(
        attributes
          .context
          .as_deref()
          .and_then(|context| context.data_directory()),
        &Self::browser_args(&attributes, &pl_attrs),
        pl_attrs.browser_extensions_enabled,
        pl_attrs.scroll_bar_style,
        attributes.incognito,
        background_color,
      );
      prestart::take(&key, hwnd)
    } else {
      None
    };
    let (env, controller) = match prestarted {
  ...
  fn create_environment(
    ...
    let additional_browser_args = Self::browser_args(attributes, &pl_attrs);
  ...
  // Accshift patch: the browser arguments, environment options and controller
  // request below are shared with prestart::start, so both make the same
  // environment.
  fn browser_args(
    attributes: &WebViewAttributes,
    pl_attrs: &super::PlatformSpecificWebViewAttributes,
  ) -> String {
    pl_attrs.additional_browser_args.clone().unwrap_or_else(|| {
      Self::default_browser_args(attributes.autoplay, attributes.proxy_config.as_ref())
    })
  }

  fn default_browser_args(autoplay: bool, proxy_config: Option<&ProxyConfig>) -> String {
  ...
  unsafe fn environment_options(
    ...
    options.set_are_browser_extensions_enabled(browser_extensions_enabled);
  ...
  fn create_controller(
    ...
    unsafe { Self::request_controller(hwnd, env, incognito, background_color, &handler)? };

    webview2_com::wait_with_pump(rx)?
  }

  unsafe fn request_controller(
    hwnd: HWND,
    env: &ICoreWebView2Environment,
    incognito: bool,
    background_color: Option<(u8, u8, u8, u8)>,
  ...
  // Accshift patch: upstream waits for each script's completion, and the first
  // one waits for the renderer process, ~50 ms before the first navigation
  // can even be asked for. The browser applies calls in the order they are
  // made, so a script added before `Navigate` still runs in that navigation's
  // documents: its completion arrives after the navigation has started, and
  // the page sees it. Nothing here reads the script id.
  #[inline]
  fn add_script_to_execute_on_document_created(webview: &ICoreWebView2, js: String) -> Result<()> {
    ...
    unsafe {
      webview.AddScriptToExecuteOnDocumentCreated(&HSTRING::from(js), &handler)?;
    }
    Ok(())
  }
